...
describe Moves::ForCastling do
  ...
  def moves_for(player)
    ...
    Moves::ForCastling.for(Rules.new(board, player, other_player))
  end

  it 'lets a player castle when no pieces are in the way' do
    expect(moves_for(player1).sort).to eq ['e1 - c1', 'e1 - g1']
    expect(moves_for(player2).sort).to eq ['e8 - c8', 'e8 - g8']
  end
  ...
  it "doesn't let a player castle if there are pieces in the way" do
    ...
    expect(moves_for(player1).sort).to eq []
    expect(moves_for(player2).sort).to eq []
  end
  ...
  it "doesn't let a player castle if they have moved their king or rook" do
    ...
    expect(moves_for(player1).sort).to eq ['e1 - g1']
    expect(moves_for(player2).sort).to eq ['e8 - g8']
  ...
  specify "a player can't castle into check" do
    ...
    expect(moves_for(player1).sort).to eq ['e1 - c1']
  end
  ...
  specify 'a player can sometimes only castle to one side' do
    ...
    expect(moves_for(player1).sort).to eq ['e1 - g1']
    expect(moves_for(player2).sort).to eq ['e8 - c8']
  end
