...
describe Moves::Knight do
  ...
  it 'knows where a knight can move' do
    ...
    expect(Moves::Knight.for(board, 'd4').sort).to eq %w[b3 b5 c2 c6 e2 e6 f3 f5]

    board.place_piece(black_pawn, 'b3')
    board.place_piece(white_pawn, 'b5')

    expect(Moves::Knight.for(board, 'd4')).to include 'b5'
    expect(Moves::Knight.for(board, 'd4')).not_to include 'b3'
  end
  ...
  it 'strips target spaces with a rank of 10' do
    ...
    expect(Moves::Knight.for(board, 'h8')).not_to include 'g10'
    expect(Moves::Knight.for(board, 'h8')).not_to include 'i10'
  end

  # regression spec
  specify 'only knights have knight moves' do
    board.place_piece(black_pawn, 'h8')
    expect(Moves::Knight.for(board, 'h8')).to eq []
  end
end
