...
describe Rules do
  ...
  describe '#valid_move?' do
    it 'knows when a move is valid' do
      expect(rules.valid_move?('d4 - d5')).to eq true
    end

    it 'is false when there is no piece in original space' do
      expect { rules.valid_move?('a4 - a5') }.to raise_error(Rules::InvalidMoveError)
    end

    it 'is false when it tries to move the wrong color piece' do
      expect { rules.valid_move?('c3 - c4') }.to raise_error(Rules::InvalidMoveError)
    end

    it 'is false when it tries to make an illegal move' do
      expect(  rules.valid_move?('d4 - d8')).to eq true
      expect(  rules.valid_move?('d4 - h4')).to eq true
      expect { rules.valid_move?('d4 - e5') }.to raise_error(Rules::InvalidMoveError)
    end
    ...
    it "doesn't let you move into check" do
      ...
      %w[a3 b3 c2 c1].each do |invalid_move|
        expect { rules.valid_move?("b2 - #{invalid_move}") }.to raise_error(Rules::InvalidMoveError)
      end

      %w[a2 a1 b1 c3].each do |valid_move|
        expect(rules.valid_move?("b2 - #{valid_move}")).to eq true
      end
    end
    ...
    it 'forces you to move out of check' do
      ...
      expect { rules.valid_move?('c2 - c1') }.to raise_error(Rules::InvalidMoveError)
      expect { rules.valid_move?('d4 - e4') }.to raise_error(Rules::InvalidMoveError)

      expect(  rules.valid_move?('c2 - b2')).to eq true
      expect(  rules.valid_move?('c2 - d1')).to eq true
      expect(  rules.valid_move?('c2 - c3')).to eq true
    end
  end
  ...
  describe '#all_moves_for_space' do
    it 'lists all moves for a piece' do
      ...
      expect(rules.all_moves_for_space('a8').sort).to eq ['a8 - a7', 'a8 - b7', 'a8 - b8']
    end

    it 'includes pawn moves' do
      board.place_piece(white_pawn, 'a2')
      expect(rules.all_moves_for_space('a2').sort).to eq ['a2 - a3', 'a2 - a4']

      board.place_piece(black_rook, 'b3')
      expect(rules.all_moves_for_space('a2').sort).to include 'a2 - b3'
    end
    ...
    it 'includes castle moves' do
      ...
      expect(rules.all_moves_for_space('e1').sort).not_to include 'e1 - c1'
      expect(rules.all_moves_for_space('e1').sort).to include 'e1 - g1'
    end
  end
  ...
  describe '#all_moves_for_player' do
    it 'lists every move a player can make' do
      ...
      expect(rules.all_moves_for_player).to include 'd4 - c5'
      expect(rules.all_moves_for_player).to include 'd4 - e3'
      expect(rules.all_moves_for_player.length).to eq 8

      board.place_piece(white_rook, 'a1')

      expect(rules.all_moves_for_player).to include 'a1 - a2'
      expect(rules.all_moves_for_player).to include 'a1 - a8'
      expect(rules.all_moves_for_player).to include 'd4 - c3'
    end
  end
end
